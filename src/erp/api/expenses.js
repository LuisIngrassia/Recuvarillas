...
*/
export const OPERATIVE_TYPES = ['produccion', 'flete']
...
export function totalsByType(expenses) {
  const totals = Object.fromEntries(EXPENSE_TYPES.map((tipo) => [tipo, 0]))
  let total = 0
  let operativos = 0
  let reinversion = 0
  ...
  for (const expense of expenses) {
    ...
    total += monto
    if (esDeReinversion(expense.tipo)) reinversion += monto
    else operativos += monto
  }

  return { ...totals, total, operativos, reinversion }
}
